...
def build_pretrain(word2idx, pretrain_dir, embed_dim, save_embedding_dir):
...
def build_dataset(path, word2idx, pad_size=0, label=None):
    dataset = []
...
if __name__ == '__main__':
    ...
    pos_data_path = os.path.join(data_dir, pos_data_path)
    neg_data_path = os.path.join(data_dir, neg_data_path)
    pretrain_path = os.path.join(data_dir, pretrain_path)
    embedding_path = os.path.join(data_dir, embedding_path)
    vocab_path = os.path.join(data_dir, vocab_path)
    train_path = os.path.join(data_dir, train_path)
    test_path = os.path.join(data_dir, test_path)

    word2idx = build_vocab((pos_data_path, neg_data_path), vocab_path, 2)

    build_pretrain(word2idx, pretrain_path, 50, embedding_path)

    pos_set = build_dataset(pos_data_path, word2idx, pad_size=32, label=1)
    neg_set = build_dataset(neg_data_path, word2idx, pad_size=32, label=0)

    train_set = pos_set[:-500] + neg_set[:-500]
    test_set = pos_set[-500:] + neg_set[-500:]

    pkl.dump(train_set, open(train_path, 'wb'))
    pkl.dump(test_set, open(test_path, 'wb'))
